...
class iaReview extends abstractModuleFront
{
    ...
    public function getByItemAndId($itemName, $itemId)
    {
        ...
        $this->iaDb->bind($stmt, ['item' => $itemName, 'id' => $itemId]);
    ...
    public function userReviewed(array $review)
    {
        ...
        $this->iaDb->bind($stmt, ['item' => $review['item_type'], 'review' => $review['id']]);
    ...
    {
        if ($settings = $this->iaDb->row(['id', 'review_allowed', 'comment_allowed'],
            iaDb::convertIds($itemName, 'item'), $this->_tableItems)
        ) {
            $options = $this->iaDb->all(['title', 'data'],
    ...
    {
        return $this->iaDb->keyvalue(['id', 'item'], iaDb::EMPTY_CONDITION, $this->_tableItems);
    }

    public function setItemSettings($itemName, $reviewsAllowed, $commentAllowed)
    {
        $stmt = iaDb::convertIds($itemName, 'item');
        $data = ['review_allowed' => (int)$reviewsAllowed, 'comment_allowed' => (int)$commentAllowed];

        $this->iaDb->setTable($this->_tableItems);

        $this->iaDb->exists($stmt)
            ? $this->iaDb->update($data, $stmt)
            : $this->iaDb->insert(array_merge($data, ['item' => $itemName]));
